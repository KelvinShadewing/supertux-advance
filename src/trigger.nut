pipeFunnel <- function (target) {
	if (
		target.x < x &&
		target.hspeed < 1 &&
		getcon("down", "hold", false, target.playerNum)
	)
		target.hspeed += 0.25;
	if (
		target.x > x &&
...
trigCurrent <- function (h = 0, v = 0, f = 0.5, eforce = false) {
	...
	if (eforce) {
		if (h > 0 && myTarget.ehspeed < h) myTarget.ehspeed += f;

		if (h < 0 && myTarget.ehspeed > h) myTarget.ehspeed -= f;

		if (v > 0 && myTarget.evspeed < v) myTarget.evspeed += f;
...
trigStep <- function (h = 0, v = 0) {
	...
	if (myTarget.placeFree(myTarget.x + h, myTarget.y)) myTarget.x += h;
	else
		for (local i = 1; i <= 4; i++) {
			if (
				myTarget.placeFree(myTarget.x + h, myTarget.y + i) &&
				!myTarget.placeFree(myTarget.x + h, myTarget.y - i)
			) {
				myTarget.x += h;
				myTarget.y += i;
	...
	if (myTarget.placeFree(myTarget.x, myTarget.y + v)) myTarget.y += v;
	else
		for (local i = 1; i <= 4; i++) {
			if (
				myTarget.placeFree(myTarget.x + i, myTarget.y + v) &&
				!myTarget.placeFree(myTarget.x - i, myTarget.y + v)
			) {
				myTarget.x += i;
				myTarget.y += v;
