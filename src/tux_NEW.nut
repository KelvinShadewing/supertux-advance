::Tux <- class extends Player {
	...
	function ruNormal() {
		...
		//Sliding controls
		if(sliding) {
			if(((!getcon("down", "hold") && !autocon.down || fabs(hspeed) < 0.5) && !placeFree(x, y + 1) && game.weapon != 3) || (fabs(hspeed) < 0.5 && (game.weapon == 4 && !getcon("shoot", "hold"))) || (game.weapon == 4 && !getcon("shoot", "hold") && !getcon("down", "hold") && !autocon.down)) if(anim == anSlide || anim == anCrawl) {
		...
		///////////////
			...
			if(anim == anCrawl) {
				...
				if(placeFree(x + 4, y + 2) && hspeed > 1 || placeFree(x - 4, y + 2) == hspeed < 1) anim = anSlide
			}
