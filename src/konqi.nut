	...
	function draw() {
		if(!hidden) {
			...
			if(anim != null) {
				...
				if(blinking == 0 || anim == anHurt) drawSpriteZ(0, sprite, anim[floor(frame)], x - camx, y - camy, 0, int(flip), 1, 1, 1)
				else drawSpriteZ(0, sprite, anim[floor(frame)], x - camx, y - camy, 0, int(flip), 1, 1, wrap(blinking, 0, 10).tofloat() / 10.0)
			}
			if(debug) {
			...
			//After image
			if((zoomies > 0 || anim == anStomp || (anim == anStatue && vspeed > 4)) && getFrames() % 2 == 0) newActor(AfterImage, x, y, [sprite, anim[wrap(floor(frame), 0, anim.len() - 1)], 0, flip, 0, 1, 1])
		}
