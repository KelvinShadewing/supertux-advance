...
getcon <- function (
	...
	if (player == 1 || player == 0) {
		...
		if (gvNumPlayers == 1 || gvGameMode != gmPlay)
			joy.index = config.joy.index;
		autonum = autocon.a;
	}
	if (player == 2) {
	...
	switch (control) {
		case "up":
			if (player == 2 && gvNetPlay) {
				...
				if (player == 1 || gvNumPlayers == 1)
					if (keyfunc(config.key.up)) return true;

				if (player == 2 || gvNumPlayers == 1 || joyCount() > 1) {
					if (
						hatfunc(joy.index, js_up) ||
						(state == "hold" &&
							joyY(joy.index) < -deadzone &&
							config.stickactive)
					)
						return true;
					if (
						state == "press" &&
						joyAxisPress(joy.index, 1, deadzone) == -1 &&
						config.stickactive
					)
						return true;
					if (
						state == "release" &&
						joyAxisRelease(joy.index, 1, deadzone) == -1 &&
						config.stickactive
					)
						return true;
				}
				...
				if (player == 0) {
					...
					for (local i = 0; i < joyCount(); i++) {
						...
						if (
							state == "press" &&
							joyAxisPress(i, 1, deadzone) == -1 &&
							config.stickactive
						)
							return true;
						if (
							state == "release" &&
							joyAxisRelease(i, 1, deadzone) == -1 &&
							config.stickactive
						)
							return true;
					}
		...
		case "left":
			if (player == 2 && gvNetPlay) {
				...
				if (player == 2 || gvNumPlayers == 1 || joyCount() > 1) {
					if (
						hatfunc(joy.index, js_left) ||
						(state == "hold" &&
							joyX(joy.index) < -deadzone &&
							config.stickactive)
					)
						return true;
					if (
						state == "press" &&
						joyAxisPress(joy.index, 0, deadzone) == -1 &&
						config.stickactive
					)
						return true;
					if (
						state == "release" &&
						joyAxisRelease(joy.index, 0, deadzone) == -1 &&
						config.stickactive
					)
						return true;
				}
				...
				if (player == 0) {
					...
					for (local i = 0; i < joyCount(); i++) {
						...
						if (
							state == "press" &&
							joyAxisPress(i, 0, deadzone) == -1 &&
							config.stickactive
						)
							return true;
						if (
							state == "release" &&
							joyAxisRelease(i, 0, deadzone) == -1 &&
							config.stickactive
						)
							return true;
					}
		...
		case "jump":
			if (gvAutoCon && useauto) {
				...
				if (keyfunc(config.key.jump) && (player == 1 || player == 0))
					return true;
		...
		case "shoot":
			if (gvAutoCon && useauto) {
				...
				if (keyfunc(config.key.shoot) && (player == 1 || player == 0))
					return true;
		...
		case "spec1":
			if (gvAutoCon && useauto) {
				...
				if (keyfunc(config.key.spec1) && (player == 1 || player == 0))
					return true;
		...
		case "spec2":
			if (gvAutoCon && useauto) {
				...
				if (keyfunc(config.key.spec2) && (player == 1 || player == 0))
					return true;
				if (joyfunc(joy.index, joy.spec2)) return true;
			}
			break;
		case "pause":
			if (keyfunc(config.key.pause) && (player == 1 || player == 0))
				return true;
		...
		case "swap":
			if (gvAutoCon && useauto) {
				...
				if (keyfunc(config.key.swap) && (player == 1 || player == 0))
					return true;
		...
		case "accept":
			if (gvAutoCon && useauto) {
				...
				if (keyfunc(config.key.accept) && (player == 1 || player == 0))
					return true;
		...
		case "upPeek":
			if (gvAutoCon && useauto) {
				...
				if (keyfunc(config.key.upPeek) && (player == 1 || player == 0))
					return true;
...
rebindKeys <- function (newkey) {
	...
	while (!done) {
		...
		switch (newkey) {
			...
			case 12:
				keyname =
					gvLangObj["controls-menu"]["cam-right-peek-selection"];
				if (tempkey != -1 && tempkey != k_escape) {
		...
		if (tempkey == k_escape) done = true;

		local message = format(
			gvLangObj["controls-menu"]["press-key-for"],
			keyname
		);
...
rebindGamepad <- function (joystep, joypad = 0) {
	...
	while (!done) {
		...
		local message = format(
			gvLangObj["controls-menu"]["press-button-for"],
			keyname
		);
...
rebindJoyPeek <- function (axis, player = 0) {
	...
	local message = format(
		gvLangObj["controls-menu"]["peek-axis"],
		gvLangObj["controls-menu"][
			axis == 0 ? "peek-horizontal" : "peek-vertical"
		]
	);
	local done = false;
	...
	while (!done) {
		...
		for (local i = 0; i < 10; i++) {
			if (
				abs(joyAxis(joy.index, i)) >= 1000 &&
				abs(joyAxis(joy.index, i)) <= 10000
			) {
...
getConName <- function (control, getkey = true, getjoy = true) {
	...
	switch (control) {
		case "up":
			if (getkey) output += gvLangObj["key"][config.key.up.tostring()];
			if (getjoy) output += "(" + chint(30) + ")";
			break;
		case "down":
			if (getkey) output += gvLangObj["key"][config.key.down.tostring()];
			if (getjoy) output += "(" + chint(31) + ")";
			break;
		case "left":
			if (getkey) output += gvLangObj["key"][config.key.left.tostring()];
			if (getjoy) output += "(" + chint(17) + ")";
			break;
		case "right":
			if (getkey) output += gvLangObj["key"][config.key.right.tostring()];
			if (getjoy) output += "(" + chint(16) + ")";
			break;
		case "jump":
			if (getkey) output += gvLangObj["key"][config.key.jump.tostring()];
			if (getjoy) {
			...
			break;
		case "shoot":
			if (getkey) output += gvLangObj["key"][config.key.shoot.tostring()];
			if (getjoy) {
				local map = gvLangObj["joy"][config["joymode"].tolower()];
			...
			break;
		case "spec1":
			if (getkey) output += gvLangObj["key"][config.key.spec1.tostring()];
			if (getjoy) {
				local map = gvLangObj["joy"][config["joymode"].tolower()];
			...
			break;
		case "spec2":
			if (getkey) output += gvLangObj["key"][config.key.spec2.tostring()];
			if (getjoy) {
				local map = gvLangObj["joy"][config["joymode"].tolower()];
			...
			break;
		case "swap":
			if (getkey) output += gvLangObj["key"][config.key.swap.tostring()];
			if (getjoy) {
				local map = gvLangObj["joy"][config["joymode"].tolower()];
			...
			break;
		case "pause":
			if (getkey) output += gvLangObj["key"][config.key.pause.tostring()];
			if (getjoy) {
				local map = gvLangObj["joy"][config["joymode"].tolower()];
