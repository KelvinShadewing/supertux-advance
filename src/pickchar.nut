...
pickCharInitialize <- function (playerNum = 1, unlockAll = false) {
	...
	foreach (key, i in gvCharacters) {
		if (key in game.characters || unlockAll) {
			...
			if (
				(playerNum == 1 && game.playerChar2 != key) ||
				(playerNum == 2 && game.playerChar != key)
			)
...
pickChar <- function () {
	...
	if (!didpick) {
		...
		if (getcon("accept", "press") || getcon("jump", "press")) {
			if (pickCharSettings.player == 1)
				game.playerChar = charlist[charslot][0];
			if (pickCharSettings.player == 2)
				game.playerChar2 = charlist[charslot][0];
			didpick = true;
	...
	drawText(
		font2,
		screenW() / 2 - gvLangObj["options-menu"]["charsel"].len() * 4,
		16,
		gvLangObj["options-menu"]["charsel"]
	);
	if (charlist[charslot][0] != "")
		drawText(
			font2,
			screenW() / 2 - charlist[charslot][4].len() * 4,
			200,
			charlist[charslot][4]
		);
	else
		drawText(
			font2,
			screenW() / 2 - gvLangObj["menu-commons"]["noone"].len() * 4,
			200,
	...
	if (charlist[charslot][0] != "") {
		if (didpick)
			drawSprite(
				getroottable()[charlist[charslot][1]],
				charlist[charslot][2][1],
				screenW() / 2,
				screenH() - 64
			);
		else
	...
	for (local i = 0; i < charlist.len(); i++) {
		if (charlist[i][0] != "")
			drawSprite(
				getroottable()[charlist[i][3]],
				0,
				listx + wrap(i, 0, 16) * 16,
				48 + floor(i / 16) * 16
			);
		else
			drawSprite(
	...
	drawSprite(
		sprCharCursor,
		getFrames() / 16,
		listx + wrap(charslot, 0, 16) * 16,
		48 + floor(charslot / 16) * 16
	);
