...
CoinSmall <- class extends PhysAct{
	...
	function draw() {
		drawSprite((
				config.bigItems ? sprCoin : sprCoinSmall),
				getFrames() / 4
				x - camx,
				y - camy - (config.bigItems ? 4 : 0),
				0,
				0,
				1,
				1,
				(timer > 60 ? 1 : float(timer) / 10
			));
	}
...
Berry <- class extends Actor{
	...
	function draw() {
		drawSprite((
			config.bigItems ? sprBerryLarge : sprBerry),
			0,
			x - camx,
...
MuffinEvil <- class extends PhysAct {
	...
	function run() {
		...
		if(gvPlayer2 && (inDistance2(x, y, gvPlayer2.x, gvPlayer2.y, 16) || hitTest(shape, gvPlayer2.shape))) {
			if(gvPlayer2.blinking > 0)
				return;
			if(gvPlayer2.x < x)
				gvPlayer2.hspeed = -1.0;
			else
				gvPlayer2.hspeed = 1.0;
...
Darknyan <- class extends PhysAct {
	...
	constructor(_x, _y, _arr = null) {
		...
		if(gvPlayer && gvPlayer.x > x)
			hspeed = 2;
		else if(gvPlayer2 && gvPlayer2.x > x)
			hspeed = 2;
		else
			hspeed = -2;
...
Starnyan <- class extends PhysAct {
	...
	function run() {
		...
		if(gvPlayer2 && inDistance2(x, y, gvPlayer2.x, gvPlayer2.y, 20)) {
...
AirFeather <- class extends PhysAct {
	...
	function draw() {
		drawSprite(sprAirFeather, getFrames() / 24, x - camx, y - camy);
	}
...
OneUp <- class extends PhysAct {
	...
	constructor(_x, _y, _arr = null) {
		...
		if(gvPlayer && gvPlayer.x > x) hspeed = -1;
		else hspeed = 1;
...
CoffeeCup <- class extends Actor{
	...
	function run() {
		if(gvPlayer && inDistance2(x, y, gvPlayer.x, gvPlayer.y + 2, 20)) {
			...
			deleteActor(id);
			if(game.ps2.subitem != "coffee")
				game.ps2.subitem = "coffee";
			else
				gvPlayer2.zoomies += 60 * 16;
			popSound(sndGulp, 0);
		}
	}
...
ShockBulb <- class extends Actor{
	...
	function draw() {
		drawSprite(sprShockBulb, getFrames() / 16, x - camx, y - camy);
	}
...
WaterLily <- class extends Actor {
	...
	function draw() {
		drawSprite(sprWaterLily, getFrames() / 16, x - camx, y - camy);
	}
...
LightCap <- class extends Actor {
	...
	function draw() {
		drawSprite(sprLightCap, getFrames() / 16, x - camx, y - camy);
	}
...
DarkCap <- class extends Actor {
	...
	function draw() {
		drawSprite(sprDarkCap, getFrames() / 16, x - camx, y - camy);
	}
