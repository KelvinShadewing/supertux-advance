...
runTimeline <- function (sequence) {
	return newActor(Timeline, 0, 0, sequence);
};

stopTimeline <- function (tln) {
	if (typeof tln == "Timeline") deleteActor(tln.id);
	if (
		typeof tln == "integer" &&
		tln in actor &&
		typeof actor[tln] == "Timeline"
	)
		deleteActor(tln);
};
	...
	"1": function (runner) {
		if (gvPlayer) {
			if (!gvPlayer.placeFree(gvPlayer.x + 4, gvPlayer.y))
				autocon.a.jump = true;
			if (
				!gvPlayer.placeFree(gvPlayer.x, gvPlayer.y + 1) &&
				gvPlayer.vspeed > 0
			)
