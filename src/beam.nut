...
BeamBug <- class extends Actor {
	...
	function run() {
		for (local i = 31; i > 0; i--) lightTrail[i] = lightTrail[i - 1];
		lightTrail[0] = [x, y];
	...
	function draw() {
		if (ghostRecordOld.len() <= 1) return;
			...
			if (
				inRange(
					distance2(
						lightTrail[i][0] - camx,
						lightTrail[i][1] - camy,
						lightTrail[i - 1][0] - camx,
						lightTrail[i - 1][1] - camy
					),
					2,
					128
				)
