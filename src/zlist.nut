...
drawSpriteZ <- function(z, sprite, frame, x, y, angle = 0, flip = 0, xscale = 1.0, yscale = 1.0, alpha = 1.0, color = 0xffffffff, mode = bm_blend) {
	// Make sure the depth slot exists
	if(!gvZList.rawin(z)) gvZList[z] <- array(0)

	// Insert the instruction
	gvZList[z].push([sprite, frame, x, y, angle, flip, xscale, yscale, alpha, color, mode])
}
