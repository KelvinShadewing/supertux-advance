...
LevelEnder <- class extends Actor {
	...
	function run() {
		if (
			((gvPlayer && gvPlayer.endMode) || !gvPlayer) &&
		...
		) {
			{
				...
				if (timer == 0 || getcon("pause", "press")) {
					...
					if (gvBattleMode) {
						...
						game.check = false;
						if (gvNextLevel == "timeattack-win")
							startPlay(
								"res/map/" + gvNextLevel + ".json",
								true,
								true
							);
						else
							startPlay(
								game.path + gvNextLevel + ".json",
								true,
								true
							);
						if (
							game.difficulty == 0 ||
							gvTAStep % game.difficulty == 0
						) {
							game.maxHealth++;
							game.ps.health++;
							game.ps2.health++;
...
endGoal <- function (speed = 0, unblock = "") {
	...
	if (levelEndRunner == 0) {
		...
		if (unblock != "" && !game.unblocked.rawin(unblock))
			game.unblocked[unblock] <- true;
		if (!game.completed.rawin(clearedLevel))
			game.completed[clearedLevel] <- true;
		if (
			game.levelCoins >= game.maxCoins &&
			!game.allCoins.rawin(clearedLevel)
		)
			game.allCoins[clearedLevel] <- true;
		if (
			game.secrets >= game.maxSecrets &&
			!game.allSecrets.rawin(clearedLevel)
		)
			game.allSecrets[clearedLevel] <- true;
		if (
			game.enemies >= game.maxEnemies &&
			!game.allEnemies.rawin(clearedLevel)
		)
			game.allEnemies[clearedLevel] <- true;
		if (!game.bestTime.rawin(clearedLevel))
			game.bestTime[clearedLevel] <- gvIGT;
		else if (game.bestTime[clearedLevel] > gvIGT)
			game.bestTime[clearedLevel] = gvIGT;
		...
		if (
			ghostRecordNew.len() < ghostRecordOld.len() ||
			ghostRecordOld.len() <= 1
		) {
			if (gvNumPlayers == 1) {
