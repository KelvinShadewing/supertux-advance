Water <- class extends Actor {
	...
	function run() {
		if (gvPlayer && hitTest(shape, gvPlayer.shape)) handleHits(gvPlayer);
		...
		if (currentColor != newColor) {
			...
			currentColor =
				(ceil(lr) << 24) | (ceil(lg) << 16) | (ceil(lb) << 8) | la;
		}
	}
		...
		switch (substance) {
			case "lava":
				...
				if (getFrames() % 8 == 0) {
					newActor(
						FlameTiny,
						target.x + target.shape.w - randInt(target.shape.w * 2),
						target.y + target.shape.h - randInt(target.shape.h * 2)
					);
					popSound(sndFlame);
				}
				break;
					...
					local c =
						actor[
							newActor(
								...
								target.x +
									target.shape.w -
									randInt(target.shape.w * 2),
								target.y +
									target.shape.h -
									randInt(target.shape.h * 2)
							)
						];
					c.vspeed = -0.2;
					popSound(sndBlurp);
			...
			case "swamp":
			case "honey":
				if ("stats" in target && target.stats.weapon == "water") break;
		...
		switch (substance) {
			case "acid":
				for (local i = 0; i < shape.w / 8; i++) {
					for (local j = 0; j < shape.h / 8; j++) {
						drawSprite(
							j == 0 ? sprAcidSurface : sprAcid,
							getFrames() / 16,
							x - shape.w - floor(camx) + i * 16,
							y - shape.h - camy + j * 16,
							0,
							0,
							1,
			...
			case "lava":
				for (local i = 0; i < shape.w / 8; i++) {
					for (local j = 0; j <= shape.h / 8; j++) {
						drawSprite(
							j == 0 ? sprLavaSurface : sprLava,
							j == 0
								? getFrames() / 16 + i * i
								: getFrames() / 16,
							x - shape.w - floor(camx) + i * 16,
							y - shape.h - camy + (j - 1) * 16,
							0,
							0,
							1,
							1,
				...
				for (local i = 0; i < shape.w / 8; i++) {
					drawLight(
						sprLightFire,
						i + getFrames() / 8,
						x - shape.w - floor(camx) + i * 16 + 8,
						y - shape.h - camy,
						getFrames() / 4 + i * 15,
						0,
						0.75 + sin(getFrames() / 30.0 + i * 15) * 0.25,
						0.75 + sin(getFrames() / 30.0 + i * 15) * 0.25
					);
				}
				break;
