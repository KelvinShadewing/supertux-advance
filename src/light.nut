...
drawLight <- function (
	...
	if (
		(gvLightScreen == gvLightScreen1 && gvLight == 0xffffffff) ||
		(gvLightScreen == gvLightScreen2 && gvLight2 == 0xffffffff)
	)
		return;
...
StaticLight <- class extends Actor {
	...
	constructor(_x, _y, _arr) {
		...
		if (1 in _arr) scale = _arr[1].tofloat();
	}
	...
	function draw() {
		if (sprite)
			drawLight(
				...
				0,
				0,
				scale * (sin(id + getFrames() / 16.0) / 16.0) + scale,
				scale * (cos(id + getFrames() / 16.0) / 16.0) + scale,
				alpha
			);
	...
	function run() {
		if (!gvSplitScreen) {
			if (
				...
				else if (weather in getroottable())
					drawWeather = getroottable()[weather];
				...
				if (weather == "0") drawWeather2 = 0;
					...
					drawWeather2 = getroottable()[weather];
