lastMouseX <- mouseX();
lastMouseY <- mouseY();

updateCursor <- function () {
	if (!config.showcursor) return; // If the cursor is disabled.

	drawSprite(sprCursor, 0, mouseX(), mouseY()); // Draw the cursor.

	if (mouseX() == lastMouseX && mouseY() == lastMouseY) return; // If the cursor hasn't moved.

	lastMouseX = mouseX();
	lastMouseY = mouseY();

	foreach (pos in menuItemsPos) {
		if (
			mouseX() >= pos.x - 3 &&
			mouseX() <= pos.x + pos.len - 3 &&
			mouseY() >= pos.y &&
			mouseY() <= pos.y + menu.size.h
		) {
			cursor = pos.index;
			return;
		}
	}
};

processCursorInput <- function () {
	if (!config.showcursor) return; // If the cursor is disabled.

	local pos =
		menuItemsPos[menu.items.len() > menu.size.ml ? cursor - cursorOffset : cursor]; // Get the position of the currently selected menu item only.
	if (
		mouseX() >= pos.x - 3 &&
		mouseX() <= pos.x + pos.len - 3 &&
		mouseY() >= pos.y &&
		mouseY() <= pos.y + menu.size.h
	) {
		if (menu.items[pos.index].rawin("disabled")) return;
		menu.items[pos.index].func();
		playSound(sndMenuSelect, 0);
		return;
	}
};
