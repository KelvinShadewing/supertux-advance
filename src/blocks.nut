...
BrickBlock <- class extends Actor {
		...
		if (gvPlayer) {
			if (v == 0) {
				...
				if (coins <= 1) {
					if ("anim" in gvPlayer)
						if (
							fabs(gvPlayer.hspeed + gvPlayer.ehspeed) >= 8 &&
							(gvPlayer.anim == "slide" ||
								gvPlayer.anim == "ball")
						)
							if (hitTest(slideshape, gvPlayer.shape)) {
								gvPlayer.vspeed = 0;
								deleteActor(id);
								newActor(WoodChunks, x, y);
								popSound(sndBump, 0);
		...
		if (gvPlayer2) {
			if (v == 0) {
				...
				if (coins <= 1) {
					if ("anim" in gvPlayer2)
						if (
							fabs(gvPlayer2.hspeed + gvPlayer2.ehspeed) >= 8 &&
							(gvPlayer2.anim == "slide" ||
								gvPlayer2.anim == "ball")
						)
							if (hitTest(slideshape, gvPlayer2.shape)) {
								gvPlayer2.vspeed = 0;
	...
	function draw() {
		drawSpriteZ(2, sprBrickBlock, 0, x - 8 - camx, y - 8 - camy + v);
	}
...
IceBlock <- class extends Actor {
	...
	function run() {
		if (gvPlayer) {
			if (gvPlayer.vspeed < 0)
				if (hitTest(shape, gvPlayer.shape) && gvPlayer.y > y + 4) {
					gvPlayer.vspeed = 0;
					tileSetSolid(x, y, oldsolid);
					deleteActor(id);
		...
		if (gvPlayer2) {
			if (gvPlayer2.vspeed < 0)
				if (hitTest(shape, gvPlayer2.shape) && gvPlayer2.y > y + 4) {
					gvPlayer2.vspeed = 0;
					tileSetSolid(x, y, oldsolid);
					deleteActor(id);
					newActor(IceChunks, x, y);
...
ItemBlock <- class extends Actor {
	...
	constructor(_x, _y, _arr = null) {
		...
		if (gvTARandomItem && item != 0 && item != null) item = randInt(13) + 1;
	}
	...
	function run() {
		...
		if (v <= -8) {
			...
			switch (item) {
				...
				case 8:
					// 1up item
					if (game.difficulty >= 3) newActor(Darknyan, x, y - 16);
					else newActor(OneUp, x, y - 16);
					break;
...
InfoBlock <- class extends Actor {
	...
	function run() {
		...
		if (
			...
			((gvPlayer &&
				!gvPlayer2 &&
				!inDistance2(x, y, gvPlayer.x, gvPlayer.y, 64)) ||
				(gvPlayer2 &&
					!gvPlayer &&
					!inDistance2(x, y, gvPlayer2.x, gvPlayer2.y, 64)) ||
				(gvPlayer &&
					gvPlayer2 &&
					!inDistance2(x, y, gvPlayer.x, gvPlayer.y, 64) &&
					!inDistance2(x, y, gvPlayer2.x, gvPlayer2.y, 64)))
	...
	function run() {
		...
		if (gvPlayer) {
			...
			if (
				...
				gvPlayer.vspeed = 1;
				vspeed = -1;
				foreach (k, i in sndPing) stopSound(sndPing[k]);
		...
		if (gothit) {
			if (hittime > 120)
				...
				drawSpriteZ(
					...
					sprTNT,
					frame,
					x - 8 - camx + (randInt(8) - 4) / 4,
					y - 8 - camy + (randInt(8) - 4) / 4
				);
		...
		if (actor.rawin("WeaponEffect"))
			foreach (i in actor["WeaponEffect"])
				if (
					...
					i.blast &&
						i.power > 1 &&
						(i.element == "fire" || i.element == "shock")
				) {
					hittime = max(hittime, 135 + game.difficulty);
					gothit = true;
					break;
...
ColorSwitch <- class extends Actor {
	...
	function run() {
		if (game.colorswitch[color] == true) return;
		...
		if (
			...
			dostr("saveGame()");
			if (actor.rawin("ColorBlock"))
				foreach (i in actor["ColorBlock"]) {
					i.filltile();
				}
		}
	...
	function draw() {
		if (game.colorswitch[color])
			drawSprite(sprColorSwitch, color * 2 + 1, x - camx, y - camy);
...
LockBlock <- class extends Actor {
	...
	function run() {
		if (
			(gvPlayer && inDistance2(x, y, gvPlayer.x, gvPlayer.y, 32)) ||
	...
	function run() {
		if (gvPlayer) {
			if (inDistance2(x, y, gvPlayer.x, gvPlayer.y, 64))
				if (game.maxRedCoins <= game.redCoins) {
					deleteActor(id);
					newActor(Poof, x, y);
					tileSetSolid(x, y, 0);
					popSound(sndBump, 0);
...
FireBlock <- class extends Actor {
	...
	function run() {
		...
		if (actor.rawin("Flame"))
			foreach (i in actor["Flame"])
				if (inDistance2(x, y, i.x, i.y, 16) && i.frame >= 6) {
					tileSetSolid(x, y, 0);
					deleteActor(id);
					newActor(Flame, x, y);
					popSound(sndFlame, 0);
...
CharSwapper <- class extends Actor {
	...
	function run() {
		...
		full =
			game.characters.rawin(character) &&
			!(
				(gvPlayer && typeof gvPlayer == character) ||
				(gvPlayer2 && typeof gvPlayer2 == character)
			);
	...
	function draw() {
		if (full || vspeed < 0) {
			...
			if (character in gvCharacters)
				drawSpriteZ(
					2,
					getroottable()[gvCharacters[character]["doll"]],
					0,
					x - camx,
					y - camy + v
				);
