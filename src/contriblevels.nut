...
selectContrib <- function () {
	...
	if (fileExists("contrib")) {
		...
		foreach (item in contrib) {
			if (
				...
				item != ".." &&
				isdir("contrib/" + item) &&
				fileExists("contrib/" + item + "/info.json")
			) {
				local data = jsonRead(
					fileRead("contrib/" + item + "/info.json")
				);
				meContribLevels.items.push({
					...
					func = function () {
						...
						for (local i = 0; i < tileSearchDir.len(); i++) {
							if (tileSearchDir[i] == "contrib/" + contribFolder)
								searchDirExists = true;
						}
						if (!searchDirExists)
							tileSearchDir.push("contrib/" + contribFolder);
						gvDoIGT = false;
						if (
							fileExists(
								"contrib/" + contribFolder + "/text.json"
							)
						) {
						...
						if (
							fileExists(
								"contrib/" + contribFolder + "/script.nut"
							)
						)
							if (!contribDidRun.rawin(contribFolder)) {
								require(
									"contrib/" + contribFolder + "/script.nut"
								);
					...
					desc = function () {
						if (
							lastLevelsCounted["contribFolder"] == contribFolder
						) {
							// Check if the same world as last frame is selected and if so, return saved data.
							return (
								"Progress: " +
								lastLevelsCounted["completed"] +
								"/" +
								lastLevelsCounted["total"] +
								" (" +
								lastLevelsCounted["percentage"] +
								"%)"
							);
						}
						...
						// Get all levels
						local contribWorldmapData = jsonRead(
							fileRead(
								"contrib/" +
									contribFolder +
									"/" +
						...
						foreach (layer in contribWorldmapData["layers"]) {
							...
							foreach (obj in layer["objects"]) {
								...
								if (
									...
									levels.push(obj["name"]);
							}
						}

						// Get completed levels count
						if (fileExists("save/" + contribFolder + ".json")) {
							local contribWorldmapSaveData = jsonRead(
								fileRead("save/" + contribFolder + ".json")
							);
							foreach (level, levelCompleted in contribWorldmapSaveData[
								"completed"
							]) {
								if (
									levelCompleted &&
									levels.find(level) != null
								)
						...
						if (levels.len() > 0)
							percentage =
								(completedLevelsCount * 100) / levels.len();

						lastLevelsCounted = {
							"contribFolder": contribFolder,
	...
	if (meContribLevels.items.len() == 0) {
		meContribLevels.items.push({
			name = function () {
				return gvLangObj["contrib-menu"]["empty"];
			},
			disabled = true
		});
	} // Sort contrib levels
